...
static char uart_getc_nb(uint32_t base) {
    volatile uint8_t *lsr = (uint8_t*)(base + UART_LSR);
    volatile uint8_t *rbr = (uint8_t*)(base + UART_RBR);
    if (*lsr & UART_LSR_DR) {
        return *rbr;
    }
    return 0;
...
void _start(void) {
    ...
    uart_puts(UART0_BASE, "[Machine2] Starting as receiver...\n");
    
    while(1) {
        char c = uart_getc_nb(UART1_BASE);
        if (c) {
            uart_puts(UART0_BASE, "[Machine2] Received: ");
            uart_putc(UART0_BASE, c);
            if (c == '\n') {
                uart_puts(UART1_BASE, "ACK\n");
            }
        }
